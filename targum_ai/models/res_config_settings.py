...
class ResConfigSettings(models.TransientModel):
    ...
    targum_instance_url = fields.Char(
        ...
        store=True,
        default="https://targum.ai",
        help="Base URL of your Targum instance (e.g., https://targum.ai)",
    )
    ...
    def get_values(self):
        ...
        existing_api_key = (
            self.env["ir.config_parameter"].sudo().get_param("targum_ai.api_key")
        )

        if not existing_api_key:
            api_key = self._generate_api_key()
            self.env["ir.config_parameter"].sudo().set_param(
                "targum_ai.api_key", api_key
            )
            res["api_key"] = api_key
        return res
